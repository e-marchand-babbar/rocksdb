...
class JniEnv final {
 ...
 public:
  [[nodiscard]]
  jbyteArray clone_slice( const Slice& slice ) const {
    auto const jlen = static_cast<jsize>( slice.size() * sizeof(char) );
    auto const jbytes = env_->NewByteArray( jlen );
    if ( jbytes == nullptr )
      return nullptr; // error
    env_->SetByteArrayRegion( jbytes, 0, jlen, reinterpret_cast<const jbyte*>(slice.data()) );
    return jbytes;
  }
